...
public class RuntimeTest : BrowserDefault
{
    ...
    public async Task ShouldAddConnectToChrome()
    {
        ...
        var sendMessage =
            await page.EvaluateExpressionAsync<string>("chrome.runtime.sendMessage.name");
        Assert.NotNull(sendMessage);

        var sendMessageUndefined = await page.EvaluateExpressionAsync<bool>(
            "chrome.runtime.sendMessage('nckgahadagoaajjgafhacjanaoiihapd', '') === undefined");
        Assert.True(sendMessageUndefined);

        var validIdWorks = await page.EvaluateExpressionAsync<bool>(
            "chrome.runtime.connect('nckgahadagoaajjgafhacjanaoiihapd') !== undefined");
        Assert.True(validIdWorks);

        var nestedToString = await page.EvaluateExpressionAsync<string>(
            "chrome.runtime.connect('nckgahadagoaajjgafhacjanaoiihapd').onDisconnect.addListener + ''");
        Assert.Equal("function addListener() { [native code] }", nestedToString);

        var noReturn = await page.EvaluateExpressionAsync<bool>(
            "chrome.runtime.connect('nckgahadagoaajjgafhacjanaoiihapd').disconnect() === undefined");
        Assert.True(noReturn);


        await AssertThrowsConnect(page,
        ...
        await AssertThrowsConnect(page,
            "Error at property 'includeTlsChannelId': Invalid type: expected boolean, found number.",
            "",
            new { IncludeTlsChannelId = 777 });
    }
    ...
    private async Task AssertThrowsConnect(IPage page, string error, params object[] args)
    {
        ...
        var ex = await Assert.ThrowsAsync<EvaluationFailedException>(async () =>
            await page.EvaluateFunctionAsync("(...args) => chrome.runtime.connect.call(...args)",
                args));
