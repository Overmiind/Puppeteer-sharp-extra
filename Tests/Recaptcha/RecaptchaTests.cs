...
public class RecaptchaTests : BrowserDefault
{
    [Fact]
    public async Task ShouldSolveInvisible()
    {
        var plugin = new RecaptchaPlugin(new TwoCaptcha(Resources.TwoCaptchaKey, new CaptchaProviderOptions()
        {
            StartTimeout = TimeSpan.FromSeconds(30), MaxPollingAttempts = 20, ApiTimeout = TimeSpan.FromMinutes(3),
        }));
    ...
    {
        var plugin = new RecaptchaPlugin(new TwoCaptcha(Resources.TwoCaptchaKey));
        var page = await LaunchAndGetPageAsync(plugin);
