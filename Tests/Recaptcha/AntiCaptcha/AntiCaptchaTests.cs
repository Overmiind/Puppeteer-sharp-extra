...
public class AntiCaptchaTests : BrowserDefault
{
    ...
    public async void ShouldThrowCaptchaExceptionWhenCaptchaNotFound()
    {
        var plugin =
            new RecaptchaPlugin(
                new PuppeteerExtraSharp.Plugins.Recaptcha.Provider.AntiCaptcha.AntiCaptcha(
                    Resources.AntiCaptchaKey));
    ...
    {
        var successElement =
            await page.QuerySelectorAsync("div[id='main'] div[class='description'] h2");
        var elementValue = await (await successElement.GetPropertyAsync("textContent"))
