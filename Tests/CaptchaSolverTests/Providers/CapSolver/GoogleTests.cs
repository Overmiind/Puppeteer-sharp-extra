...
public class GoogleTests : BrowserDefault
{
    ...
    public async Task ShouldSolveCheckbox()
    {
        ...
        var answerElement = await page.EvaluateExpressionAsync<string>("document.querySelector(\"body > main > h2:nth-child(3)\").textContent");
    ...
    {
        var plugin = new CaptchaSolverPlugin(new PCapSolver.CapSolver(Resources.CapSolverKey));
        var page = await LaunchAndGetPageAsync(plugin);
